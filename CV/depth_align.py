...
LEFT_RIGHT_RESOLUTION = dai.MonoCameraProperties.SensorResolution.THE_400_P # 640x400
...
# original rgb calibration parameters
...
monoLeftIntrinsics = calibrationHandler.getCameraIntrinsics(LEFT_SOCKET, 640, 400)
monoLeftDistortion = calibrationHandler.getDistortionCoefficients(LEFT_SOCKET)
distortionModel = calibrationHandler.getDistortionModel(LEFT_SOCKET)
if distortionModel != dai.CameraModel.Perspective:
    raise RuntimeError("Unsupported distortion model for mono LEFT camera. This example supports only Perspective model.")
...
#left.out.link(align.inputAlignTo)
    ...
    while True:
        ...
        if frameDepth is not None:
            ...
            blended = cv2.addWeighted(
                ...
                leftCv, rgbWeight, alignedDepthColorized, depthWeight, 0
            )
            cv2.putText(
                blended,
